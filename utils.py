...
def plot_im_with_bbox(im: Tensor, bbox: list, title=""):
...
def make_image_pyramid(im, min_pyramid_size=12, reduction_factor=0.7):
    im_pyramid = []
...
def random_crop_and_update_bbox(image, bbox, output_size):
    ...
    new_bbox[2] = min(max(bbox[0] + bbox[2] - x_crop, 0), max(x_crop + crop_width - bbox[0], 0), crop_width)  # Update width
    new_bbox[3] = min(max(bbox[1] + bbox[3] - y_crop, 0), max(y_crop + crop_height - bbox[1], 0), crop_height)  # Update height
